...
def fast_gradient_method(model_fn, model_preprocess, x, eps, norm, clip_min=None, clip_max=None, y=None,
                         targeted=False, top_k=None, sanity_checks=False):
  """
  ...
  if y is None and top_k is None:
    # Transform image to vgg format: model_preprocess(x*255)
    # Using model predictions as ground truth to avoid label leaking
    y = tf.argmax(model_fn(model_preprocess(x*255)), 1)
  elif top_k is not None:
    _, y = tf.math.top_k(model_fn(model_preprocess(x*255)), k=top_k)
    y = y[:, top_k-1]
    targeted = True
    print(y)

  grad = compute_gradient(model_fn, model_preprocess, x, y, targeted)
...
# @tf.function
def compute_gradient(model_fn, model_preprocess, x, y, targeted):
  """
  ...
  with tf.GradientTape() as g:
    ...
    # Transform image to vgg format: model_preprocess(x*255)
    # Compute loss
    loss = loss_fn(labels=y, logits=model_fn(model_preprocess(x*255)))
